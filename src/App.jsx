...
import { 
  ...
  Loader2, Database, BarChart3, PieChart, Activity
} from 'lucide-react';
...
export default function App() {
  ...
  // Состояние навигации (Чаты или Аналитика)
  const [currentTab, setCurrentTab] = useState('chats'); // 'chats' | 'analytics'
  ...
  // Проверка для индикатора "ИИ печатает..."
  const isAiTyping = activeLead?.status === 'ai_active' && 
      ...
      <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center p-6 text-left font-sans relative overflow-hidden">
        ...
        <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl max-w-sm w-full text-center border-b-8 border-blue-600 z-10 relative">
           ...
           <div className="w-24 h-24 bg-white border border-slate-100 rounded-3xl mx-auto flex items-center justify-center mb-6 shadow-xl overflow-hidden relative group">
              {/* Пытаемся загрузить картинку. Если её нет - показываем резервную иконку */}
              <img 
                src="/logo-a-1.png" 
                alt="Aegis Logo" 
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans text-slate-900 text-left overflow-hidden">
      ...
      {/* ЛЕВАЯ ПАНЕЛЬ (НАВИГАЦИЯ И СПИСОК) */}
      <aside className="w-80 border-r border-slate-200 flex flex-col shrink-0 bg-white shadow-xl z-20">
        <header className="h-auto flex flex-col px-6 pt-6 pb-4 border-b shrink-0 bg-white">
           <div className="flex items-center gap-3 mb-6">
             ...
             <div>
               <h2 className="font-black text-sm uppercase tracking-widest text-slate-800 leading-none">Aegis CRM</h2>
               <p className="text-[9px] font-bold text-slate-400 tracking-[0.2em] mt-1">Control Center</p>
             </div>
           </div>

           {/* Вкладки: Чаты / Аналитика */}
           <div className="flex bg-slate-100 p-1 rounded-xl">
             <button 
                onClick={() => setCurrentTab('chats')}
                className={`flex-1 flex justify-center items-center gap-2 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${currentTab === 'chats' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
             >
               <MessageSquare size={14}/> Чаты
             </button>
             <button 
                onClick={() => setCurrentTab('analytics')}
                className={`flex-1 flex justify-center items-center gap-2 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${currentTab === 'analytics' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
             >
        ...
        {currentTab === 'analytics' && (
           <div className="flex-1 overflow-y-auto p-6 bg-slate-50 flex flex-col items-center justify-center text-slate-400">
              <PieChart size={48} className="mb-4 opacity-50" />
              <p className="text-xs font-black uppercase tracking-widest text-center">Аналитика открыта<br/>в главном окне</p>
           </div>
        )}
      ...
      {/* ЦЕНТРАЛЬНАЯ ПАНЕЛЬ (СООБЩЕНИЯ ИЛИ АНАЛИТИКА) */}
      <main className="flex-1 flex flex-col bg-slate-100 overflow-hidden text-left z-10 relative">
         
         {currentTab === 'analytics' ? (
           <div className="p-10 overflow-y-auto w-full h-full custom-scrollbar">
             <header className="mb-10">
               <h2 className="text-3xl font-black text-slate-800 tracking-tighter uppercase leading-none">Аналитика AI-Центра</h2>
               <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-3 flex items-center gap-2">
                  <Activity size={12} className="text-blue-500"/> Статистика работы ИИ и операторов
               </p>
             </header>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
               ...
               <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col justify-between">
                 ...
                 <p className="text-[9px] font-bold text-slate-400 mt-2">Лидов в обработке бота</p>
               </div>
               <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col justify-between">
                 <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">У операторов</p>
                 <div className="text-4xl font-black text-green-500">{leads.filter(l => l.status === 'operator_active').length}</div>
                 <p className="text-[9px] font-bold text-slate-400 mt-2">Диалогов перехвачено</p>
               </div>
               <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 flex flex-col justify-between">
                 <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Всего сообщений</p>
                 <div className="text-4xl font-black text-blue-600">{messages.length}</div>
                 <p className="text-[9px] font-bold text-slate-400 mt-2">В базе данных</p>
               </div>
             </div>

             <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
               <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest mb-6">Сводка по качеству</h3>
               <div className="space-y-4">
                 <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                   <span className="text-xs font-bold text-slate-500">Среднее кол-во сообщений на лида</span>
                   <span className="font-black text-slate-800 bg-slate-100 px-3 py-1 rounded-lg">
                     {leads.length > 0 ? Math.round(messages.length / leads.length) : 0} шт.
                   </span>
                 </div>
                 <div className="flex justify-between items-center pb-4 border-b border-slate-50">
                   <span className="text-xs font-bold text-slate-500">Доля вмешательства человека</span>
                   <span className="font-black text-slate-800 bg-slate-100 px-3 py-1 rounded-lg">
                     {leads.length > 0 ? Math.round((leads.filter(l => l.status === 'operator_active').length / leads.length) * 100) : 0}%
                   </span>
                 </div>
               </div>
             </div>
           ...
           <>
             <header className="h-20 border-b flex items-center justify-between px-10 shrink-0 bg-white shadow-sm">
                <div>
                   ...
                   <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-2 flex items-center gap-2">
                      ...
                      Статус: {activeLead?.status === 'operator_active' ? 'Управление оператора' : 'Работает ИИ'} • ID {selectedId}
                   </p>
                </div>
                <div className="flex gap-3">
                   {activeLead?.status === 'operator_active' ? (
                     <button onClick={returnControl} className="px-6 py-3 bg-green-500 hover:bg-green-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all shadow-xl shadow-green-500/20 flex items-center gap-2">
                       <Bot size={14} /> Вернуть ИИ
                     </button>
                   ) : (
                     <button onClick={takeControl} className="px-6 py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all shadow-xl shadow-amber-500/20 flex items-center gap-2">
             ...
             <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar relative">
                ...
                {isAiTyping && (
                  <div className="flex justify-start opacity-70 animate-pulse transition-all">
                     <div className="bg-white border border-slate-200 text-slate-500 p-4 rounded-[2.5rem] rounded-bl-none shadow-sm flex items-center gap-3">
                        <Bot size={14} className="text-blue-500" />
                        <span className="text-[10px] font-black uppercase tracking-widest">ИИ формулирует ответ...</span>
                     </div>
                  </div>
                )}
      ...
      {/* ПРАВАЯ ПАНЕЛЬ (ПРОФИЛЬ) - Скрываем, если открыта аналитика */}
      {currentTab === 'chats' && (
